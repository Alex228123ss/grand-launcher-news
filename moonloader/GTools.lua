...
function PerformUpdate()
    ...
    downloadUrlToFile(UPDATE_LUA_URL, script_path, function(id, status)
        if status == 58 then
            sampAddChatMessage("{FFFFFF}[{FFA500}G{FFFFFF}Tools] {008000}Вы установили обновление.", -1)
...
function main()
    ...
    sampAddChatMessage("{FFFFFF}[{FFA500}G{FFFFFF}Tools] {FFFFFF}Скрипт {00FF00}успешно{FFFFFF} загружен.", -1)
    sampAddChatMessage("{FFFFFF}[{FFA500}G{FFFFFF}Tools] {FFFFFF}Панель управления: {00BFFF}/gtools{FFFFFF}.", -1)
    ...
    while true do
        ...
        if wasKeyPressed(vk.VK_RSHIFT) and not sampIsChatInputActive() and not sampIsDialogActive() then
            ...
            if Cheat.airbrake[0] then
                ...
                sampAddChatMessage('[{FFA500}G{FFFFFF}Tools] AirBrake {00DD00}включён', -1)
            else
                sampAddChatMessage('[{FFA500}G{FFFFFF}Tools] AirBrake {FF0000}выключен', -1)
        ...
        if imgui.Begin("##Tablet", nil, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
            ...
            elseif not UI.access_granted then
                ...
                if update_status == 1 then
                    ...
                    if imgui.Button(u8"Обновить", imgui.ImVec2(180, 30)) then
                        PerformUpdate()
                        update_status = 2
                    end
                    imgui.Dummy(imgui.ImVec2(0, 10))
